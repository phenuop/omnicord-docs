...
export type CommandDetailedInfo = {
  whatItDoes?: string;
  ...
  currency?: { tyden: string; nullite: string };
  boxTypes?: { name: string; description: string }[];
  ranks?: { name: string; emoji: string; rating: string; color: string }[];
  ...
  cost?: string;
  items?: string[];
  shopTypes?: { name: string; description: string }[];
};
...
export type Command = {
  ...
  examples: string[];
  tips: string[];
  detailedInfo?: CommandDetailedInfo;
};
...
export const categories = [...new Set(commandData.map(c => c.category))].sort();